			...
			for (int y = -BLOCK_SCAN_RADIUS; y <= BLOCK_SCAN_RADIUS; y++) {
				for (int z = -BLOCK_SCAN_RADIUS; z <= BLOCK_SCAN_RADIUS; z++) {
					...
					if (config.targetDiamond > 0
						...
						name = "Diamond Ore";
						priority = 20;
					} else if (config.targetEmerald > 0
							&& (state.is(Blocks.EMERALD_ORE) || state.is(Blocks.DEEPSLATE_EMERALD_ORE))) {
						category = "Emerald";
						name = "Emerald Ore";
						priority = 25;
					} else if (config.targetGold > 0 && (state.is(Blocks.GOLD_ORE)
							|| state.is(Blocks.DEEPSLATE_GOLD_ORE) || state.is(Blocks.NETHER_GOLD_ORE))) {
						category = "Gold";
						name = "Gold Ore";
						priority = 35;
					} else if (config.targetIron > 0
						...
						name = "Ancient Debris";
						priority = 15;
					} else if (config.targetChest > 0 && (state.is(Blocks.CHEST) || state.is(Blocks.TRAPPED_CHEST)
							|| state.is(Blocks.BARREL) || state.is(BlockTags.SHULKER_BOXES))) {
						category = "Chest";
