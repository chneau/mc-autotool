...
public class AutoDeposit {
    ...
    private void setupButton(Minecraft client, AbstractContainerScreen<?> containerScreen, Screen screen) {
        ...
        if (mode == Config.DepositMode.OFF) return;

        var menu = containerScreen.getMenu();
        boolean isFurnace = menu instanceof AbstractFurnaceMenu;
        
        if (isFurnace) {
            if (mode != Config.DepositMode.FURNACE && mode != Config.DepositMode.ALL) return;
        } else {
            if (mode != Config.DepositMode.CHEST && mode != Config.DepositMode.ALL) return;
        }
